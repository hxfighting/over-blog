                ...
                <div class="col-xs-12 col-md-12 col-lg-8">
                    @if(!empty($tag_name))
                        <div class="row b-tag-title">
                            <div class="col-xs-12 col-md-12 col-lg-12">
                                <h3>拥有<span class="b-highlight">{{ $tag_name }}</span>标签的文章</h3>
                            </div>
                        </div>
                    @endif
                    @if(request()->has('search'))
                        <div class="row b-tag-title">
                            <div class="col-xs-12 col-md-12 col-lg-12">
                                <h3>搜索到的与<span
                                            class="b-highlight">{{ mb_strimwidth(request()->input('search'),0,20,'...','utf8') }}</span>相关的文章
                                </h3>
                            </div>
                        </div>
                    @endif
                                        ...
                                        <div id="contact_form">
                                            @if(request()->input('search'))
                                                <p style="text-align: center;font-size: large">
                                                    博主很懒,暂时没有写关于"{{mb_strimwidth(request()->input('search'),0,13,'...','utf-8')}}
                                                    "的文章,看看其它文章吧！</p>
                                            @else
                                                <p style="text-align: center;font-size: large">
                                                    博主很懒,暂时没有写关于"{{$tag_name}}
                                                    "的文章,看看其它文章吧！</p>
                                            @endif
                                        </div>
                                    </div>
