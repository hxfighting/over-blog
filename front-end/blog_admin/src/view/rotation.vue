        ...
        <card>
            <div class="search-con search-con-top">
                <Button @click="modal_add=true" class="search-btn" type="primary">
                    <Icon type="search"/>&nbsp;&nbsp;新增轮播图
                </Button>
            </div>
            <Table border :data="tableData" :columns="tableColumns" stripe :loading="loading"></Table>
            <Modal v-model="modal_update" scrollable :mask-closable="false" :closable="false" title="修改轮播图" @on-ok="updateRotation" :loading="button_loading" @on-cancel="modalCancel">
                <Form ref="formCustom" :model="formCustom" :label-width="80">
                    ...
                    <FormItem label="轮播图文字">
                        <Input v-model="formCustom.words" type="textarea" :rows="6"></Input>
                    </FormItem>
                </Form>
            </Modal>
            <Modal v-model="modal_add" scrollable :mask-closable="false" :closable="false" title="新增轮播图" @on-ok="addRotation" :loading="button_loading" @on-cancel="modalCancel">
                <Form ref="formCustom" :model="formCustom" :label-width="80">
                    <FormItem label="轮播图图片">
                        <Spin size="large" fix v-if="spinShow"></Spin>
                        <div class="demo-upload-list" v-if="formCustom.image_url!=null">
    ...
    export default {
            ...
            return {
                ...
                tableColumns: [
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            const row = params.row;
                            const text = row.image.words==null?'无':row.image.words
                            return h('span', {
                                style: {
                                    width: "100px",
                                    verticalAlign: "middle",
                            ...
                            return h('div', [
                                ...
                                h('Poptip', {
                                    ...
                                    on: {
                                        'on-ok': () => {
                                            let data = params.row;
                                            this.deleteRotation(data.image.id)
                                        }
                                    }
                                }, [
                                    h('Button', {
        ...
        methods: {
            ...
            updateRotation(){
                ...
                updateRotation(this.formCustom).then(res=>{
                    let data = res.data;
                    if(data.code==200){
            ...
            addRotation(){
                ...
                addRotation(this.formCustom).then(res=>{
                    let data = res.data;
                    if(data.code==200){
